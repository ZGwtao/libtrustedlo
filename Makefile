...
$(PANCAKE_SELF_LOAD_ASM): $(PANCAKE_SELF_LOAD_SRC) $(PNK_ARGS_HDR) $(PNK_MEM_SRC) $(PNK_ELF_SRC) | $(LIB_BUILD_DIR)
	$(PANCAKE_CPP) -P -include $(PNK_MEM_SRC) -include $(PNK_ELF_SRC) $< | $(PANCAKE_COMPILER) $(PANCAKE_FLAGS) > $@
